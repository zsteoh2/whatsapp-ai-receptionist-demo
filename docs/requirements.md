...
| REQ-AUTH-001 | Must | Authenticate administrative HTTP actions with Supabase Auth. | Bearer access tokens are verified with Supabase; `/api/me` and Google OAuth initiation reject missing, invalid, or expired tokens. | approved |
| REQ-PRES-001 | Must | Present the assistant as ORA with a clean, reusable prospect journey. | The welcome invites prospects to explore ORA's capabilities, business knowledge, booking/payment/follow-up automation, and Founder contact; `START DEMO` enters an explicitly synthetic name → availability → policy → £1 Stripe Test Mode payment → confirmation → Founder journey; Clinic terms never appear in ORA-only mode. | approved |
| REQ-PRES-003 | Must | Demonstrate ORA's own knowledge without inventing a customer business. | ORA-specific AI understands natural questions and demo/detail intents using current booking fields and last-topic context; approved truthful capability replies preserve progress. Model failure does not guess fields or execute actions. The generic interactive demo is labelled synthetic and creates no real appointment; short explicit conversation endings provide the Founder CTA and retest instruction. | approved |
