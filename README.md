...
The webhook validates `X-Twilio-Signature`. Never paste the Auth Token into chat, source control, screenshots, or commands that will be shared. Fast incoming conversations reply synchronously with TwiML. If processing takes over 8 seconds, the webhook acknowledges with empty TwiML and the eventual reply uses the outbound Messages API, which requires sender/account permission for free-form replies. Delayed work runs in-process and is not durable across restarts.
